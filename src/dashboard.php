                        ...
                        foreach ($columns as $column) {
                            ...
                            ?>
                                    ...
                                    <td>
                                        ...
                                        <div class="gradient-sticky-bar" style="background-color: green; width: 80%; --percentage: <?= $dataQuality['correct_data_percentage'] ?? 0?>%;" data-url="view_table.php?column=<?=$column["column_name"]?>&table=<?=$tableName?>&project=<?=$projectName?>"></div>
                                            <div class="sticky-bar-1" style="background-color: green; width: 10%; border-radius:3px;font-size:10px;text-align:center;"><a href="view_table.php?column=<?=$column["column_name"]?>&table=<?=$tableName?>&project=<?=$projectName?>" style="color:white;text-decoration:none;"><?=$dataQuality['correct_data_percentage'] ?? 0?>%</a></div>
                                        </div>
                                    </td>
                                    <td>
                                    <div class="sticky-bar-container">
    ...
    options: {
        ...
        onClick: (evt, elements) => {
            if (elements.length > 0) {
                ...
                const urlMap = {
                    'Correct data': '<?=WEBSITE_ROOT_PATH?>view_dataquality.php?table=<?=$tableName?>&project=<?=$projectName?>&type=correct',
                    'Wrong data': '<?=WEBSITE_ROOT_PATH?>view_dataquality.php?table=<?=$tableName?>&project=<?=$projectName?>&type=incorrect',
                };
                const url = urlMap[label];
                console.log(url);
                if (url) {
...
document.querySelectorAll('.gradient-sticky-bar').forEach(function(element) {
